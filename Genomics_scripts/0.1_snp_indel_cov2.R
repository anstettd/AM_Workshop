##################################################################################
## Calculating Mean snp and indel coverage for second round 
## Author Daniel Anstett
## this is the final script for this part
##
...
baseline<-read.table("Genomics_scripts/Data/cov_baseline.tsv", sep = "\t", fill = TRUE , header = TRUE)
baseline_cov_summary <- data.frame()

global_mean_snp <- mean(baseline$DP)
global_sd_snp <- sd(baseline$DP)
upper_dp_snp <- global_mean_snp + global_sd_snp 
lower_dp_snp <- global_mean_snp - global_sd_snp 
upper_dp_snp
lower_dp_snp
