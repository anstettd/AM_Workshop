...
c_range <- st_read("Shape/c_range.shp") 
...
#Baseline & Timeseries
baseline_pop <- read_csv("SDM/data_files/presences.csv")
...
baseline_pop <- baseline_pop  %>% dplyr::select(Longitude,Latitude)
baseline_pop_sf <- st_as_sf(baseline_pop,coords=c("Longitude","Latitude"), crs=EPSG4326)
##############################################################################
...
#plot polygon on mape

tmap_mode("plot")
#tmap_mode("view")
c_range_plot <- tm_shape(c_range, bbox=st_bbox(calo))
tm_shape(baseline_pop_sf)+
tm_dots(size=0.2,shape=1)+
c_range_plot 
