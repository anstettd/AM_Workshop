##################################################################################
## Region regression plots of BF>20 SNPs for random and observed 2010-2014
...
###################################################################################
#Setup plots
joint_env_p1 <- joint_env %>% filter(Site=="North",Year<=2014) 
joint_env_p2 <- joint_env %>% filter(Site=="Centre",Year<=2014)
joint_env_p3 <- joint_env %>% filter(Site=="South",Year<=2014)
...
ggsave("Graphs_CI/Frequency/env_both_freqchange_paper_North1014.pdf",width=12, height = 6, units = "in")
...
ggplot(data=joint_env_p2,aes(Year,SNP_Freq,group=SNP_ID,color=Type)) + 
  ...
  theme(legend.title = element_blank(),
        ...
        strip.background = element_blank(), 
        strip.text.x=element_text(size=14,face="bold",hjust=0,vjust=-1.2))
ggsave("Graphs_CI/Frequency/env_both_freqchange_paper_Center1014.pdf",width=12, height = 6, units = "in")
...
ggplot(data=joint_env_p3,aes(Year,SNP_Freq,group=SNP_ID,color=Type)) + 
  ...
  theme(legend.title = element_blank(),
        ...
        strip.background = element_blank(), 
        strip.text.x=element_text(size=14,face="bold",hjust=0,vjust=-1.2))
ggsave("Graphs_CI/Frequency/env_both_freqchange_paper_South1014.pdf",width=12, height = 6, units = "in")
