...
spp_shp = SpatialPointsDataFrame(spp_shp, data.frame(ID=1:nrow(spp_locality),Species=spp_locality$species_name))

proj4string(spp_shp) <- CRS(proj.xy)
...
temp10k2 <- gBuffer(temp50k, width = -40000) # reduce the buffer (-40km)
...
pts_temp = spp_locality[,c("Species","Longitude","Latitude")] # need to be this order
